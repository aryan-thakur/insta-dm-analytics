...
DATABASE_FILENAME = os.environ.get("DATABASE_FILENAME", "instagram_messages.db") # Default if env var is not set
...
def message_volume():
    ...
    try:
        query = db.query(
            func.strftime('%Y-%m', Message.timestamp_iso).label('month'), # Use timestamp_iso
        ...
        fig.update_layout(title=f'Message Volume Per Month{" for " + username_filter if username_filter else ""}',
                          xaxis_title='Month',
        ...
        """, graph_html=graph_html, username=username_filter)
